...
# artificial prior
prior <- list()
prior$logdensity <- function(x) as.numeric(coxprocess_log_vi_proposal(x))
prior$gradlogdensity <- function(x) coxprocess_gradlog_vi_proposal(x)
prior$rinit <- function(n) coxprocess_sample_vi_proposal(n)
...
likelihood$logdensity <- function(x) as.numeric(coxprocess_logprior(x)) + 
  ...
  as.numeric(coxprocess_log_vi_proposal(x))
likelihood$gradlogdensity <- function(x) coxprocess_gradlogprior(x) + 
  coxprocess_gradloglikelihood(x, data_counts) - 
  coxprocess_gradlog_vi_proposal(x)

# define functions to compute gibbs flow (and optionally velocity)
exponent <- 2
compute_gibbsflow <- function(stepsize, lambda, lambda_next, derivative_lambda, x, logdensity) coxprocess_gibbsflow_vi_proposal(stepsize, lambda, derivative_lambda, x, logdensity, data_counts)
gibbsvelocity <- function(t, x) as.matrix(coxprocess_gibbsvelocity_vi_proposal(t, x, exponent, data_counts))
...
gnormvelocity
save(normvelocity.df, file = "inst/coxprocess/results/normvelocity_gfais_vi_400.RData")
# ggsave(filename = "~/Dropbox/GibbsFlow/draft_v3/coxprocess_normvelocity_gfais_vi_400.pdf", plot = gnormvelocity,
