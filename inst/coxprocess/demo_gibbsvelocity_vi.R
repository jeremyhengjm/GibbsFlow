...
gibbs_velocity <- function(t, x){
  x <- matrix(x, nrow = 1)
  output <- as.matrix(coxprocess_gibbsvelocity_vi_proposal(t, x, exponent, data_counts))
  return(output)
}
...
save(min_stepsize, total_nsteps, file = "inst/coxprocess/results/gibbsflow_integration_vi_400.RData", safe = F)
