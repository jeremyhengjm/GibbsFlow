...
const double nobservations = 18.0;
...
// using the ordering x[0] = sigma_{theta}^2, x[1] = mu, x[2:19] = theta

// Inverse Gamma distribution
double invgamma_logdensity(double x, double alpha, double beta){
  double output = alpha * std::log(beta) - std::lgamma(alpha) - (alpha + 1.0) * std::log(x) - beta / x;
  return(output);
}

// Improper inverse Gamma distribution
double invgamma_improper_logdensity(double x, double beta){
...
double gaussian_logdensity(double x, double mean, double variance){
  double xc, output;
  xc = x - mean;
...
arma::vec baseball_artificial_logprior(arma::mat x){
  ...
  for (int n = 0; n < N; n++){
    ...
    mu = x(n, 1);
    logdensity = 0;
    logdensity = invgamma_logdensity(sigma_theta_sq, alpha_1, beta_1);
    logdensity += gaussian_logdensity(mu, mu_1, sigma_1_sq);
    for (int i = 2; i < dimension; i++){
      theta_i = x(n, i);
      logdensity += gaussian_logdensity(theta_i, mu_2, sigma_2_sq);
    }
    output(n) = logdensity;
...
arma::vec baseball_logprior(arma::mat x){
  ...
  for (int n = 0; n < N; n++){
    ...
    mu = x(n, 1);
    logdensity = 0;
    logdensity = invgamma_improper_logdensity(sigma_theta_sq, beta_0);
    for (int i = 2; i < dimension; i++){
      theta_i = x(n, i);
      logdensity += gaussian_logdensity(theta_i, mu, sigma_theta_sq);
    }
    output(n) = logdensity;
...
// Likelihood
// load baseball dataset from Table 1 of Morris (1983)
arma::vec baseball_dataset(){
...
arma::vec dataset = baseball_dataset();
...
arma::vec baseball_loglikelihood(arma::mat x){
  ...
  for (int n = 0; n < N; n++){
    loglikelihood = 0;
    for (int i = 2; i < dimension; i++){
      obs_minus_theta = dataset(i-2) - x(n, i);
...
arma::mat baseball_gradloglikelihood(arma::mat x){
  ...
  for (int n = 0; n < N; n++){
    for (int i = 2; i < dimension; i++){
      output(n, i) = (dataset(i-2) - x(n, i)) * inv_sigma_e_sq;  
    }
  }
  return(output);
}
...
//' @export
// [[Rcpp::export]]
Rcpp::List baseball_gibbsflow(double stepsize, double lambda, double lambda_next, double derivative_lambda,
  ...
  for (int n = 0; n < N; n++){
    ...
    for (int i = 2; i < dimension; i++){
      ...
      current_mean = (sigma_e_sq * temp_m + lambda * temp_s_sq * dataset(i-2)) / combined_var;
      ...
      next_mean = (sigma_e_sq * temp_m + lambda_next * temp_s_sq * dataset(i-2)) / combined_var;
...
arma::mat baseball_gibbsvelocity(double time, arma::mat xparticles, double exponent){
  ...
  for (int n = 0; n < N; n++){
    ...
    for (int i = 2; i < dimension; i++){
      ...
      // xi summary
      xi_numerator = sigma_e_sq * m_summary + lambda * s_sq * dataset(i-2);
      xi_denominator = sigma_e_sq + lambda * s_sq;   
      xi_summary = xi_numerator / xi_denominator;
      
      d_xi_numerator = sigma_e_sq * d_m_summary + (derivative_lambda * s_sq + lambda * d_s_sq) * dataset(i-2);
